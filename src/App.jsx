...
function App() {
  ...
  async function handleImages(files, photoType = productForm.nextPhotoType || 'Other Evidence Photo', sourceType = 'uploaded') {
    ...
    try {
      const images = await Promise.all(selected.map(async (file) => ({ id: id(), name: file.name || photoType, type: photoType, sourceType, createdAt: now(), dataUrl: await imageToLocalDataUrl(file) })));
