/**
 * BrandTrace barcode detection utilities.
 * Strategy (in order):
 * 1. Native BarcodeDetector when available
 * 2. Local pure-JS EAN/UPC linear fallback (no dependency)
 * 3. ZXing (@zxing/browser) as robust cross-browser fallback
 *
 * All processing stays in the browser. Only the decoded barcode number
 * is ever sent to a public product lookup service when the user chooses.
 */
...
function bitsFromRow(ctx, y, width) {
  ...
  let first = raw.findIndex(Boolean);
  let last = raw.length - 1 - [...raw].reverse().findIndex(Boolean);
  if (first < 0 || last <= first) return '';
  const modules = 95;
  const moduleWidth = (last - first + 1) / modules;
  let bits = '';
  for (let i = 0; i < modules; i++) {
    const a = Math.max(0, Math.round(first + i * moduleWidth));
    const b = Math.min(width, Math.round(first + (i + 1) * moduleWidth));
    let sum = 0;
    for (let x = a; x < b; x++) sum += raw[x];
    bits += sum >= (b - a) / 2 ? '1' : '0';
  }
  return bits;
}
...
function decodeEanBits(bits) {
  ...
  let parity = '';
  for (let i = 0; i < 6; i++) {
    const part = bits.slice(3 + i * 7, 10 + i * 7);
    let d = L.indexOf(part);
    if (d >= 0) {
      left += d;
      parity += 'L';
      continue;
...
async function localLinearFallback(source) {
  ...
  if (!w || !h) return null;
  canvas.width = Math.min(900, w);
  canvas.height = Math.max(1, Math.round((h * canvas.width) / w));
  ...
  for (const y of [0.42, 0.5, 0.58].map((v) => Math.round(canvas.height * v))) {
    ...
    if (decoded) {
      return {
        rawValue: decoded.startsWith('0') ? decoded.slice(1) : decoded,
        format: decoded.startsWith('0') ? 'upc_a' : 'ean_13',
        decoder: 'local EAN/UPC canvas fallback',
      };
    }
  }
  return null;
...
export async function getNativeDetector() {
  ...
  try {
    ...
    const formats = available?.length
      ? SUPPORTED_FORMATS.filter((f) => available.includes(f))
      : SUPPORTED_FORMATS;
...
async function getZxingReader() {
  ...
  zxingReaderPromise = (async () => {
    try {
      ...
      console.warn('ZXing browser fallback unavailable', err);
      return null;
    }
  })();
...
/**
 * Detect a barcode from a video element, canvas, or Image.
 * Returns { rawValue, format, decoder } or null.
 */
export async function detectBarcodeFromSource(source) {
  ...
  // 2. Fast local EAN/UPC linear decoder
  try {
  ...
  if (!reader) {
    throw new Error(
      'Barcode decoder unavailable. This browser does not expose BarcodeDetector and the ZXing fallback could not load.'
    );
  }

  try {
    // ZXing works best with an HTMLImageElement or canvas in many cases.
    // For live video we draw a frame first.
    if (source instanceof HTMLVideoElement) {
      ...
      return {
        ...
        format: String(result.getBarcodeFormat?.() || 'unknown').toLowerCase(),
        decoder: 'ZXing browser fallback',
      };
    }
...
export function scannerErrorMessage(error) {
  ...
  if (typeof window !== 'undefined' && !window.isSecureContext) {
    return 'Secure HTTPS context required. Camera access works only on HTTPS or localhost; manual entry and photo upload still work.';
  }
  if (typeof navigator !== 'undefined' && !navigator.mediaDevices?.getUserMedia) {
    return 'Browser camera API unsupported. Enter the UPC manually or upload a barcode photo.';
  }
  if (name === 'NotAllowedError' || name === 'SecurityError') {
    return 'Camera permission denied. You can still enter the UPC manually or upload a barcode photo.';
  }
  if (name === 'NotFoundError' || name === 'DevicesNotFoundError') {
    return 'No camera available. Use manual UPC entry or upload a barcode photo.';
  }
  if (name === 'NotReadableError' || name === 'TrackStartError') {
    return 'Camera already in use or unavailable. Close other camera apps, then try again.';
  }
  if (/decoder unavailable/i.test(text)) return text;
  if (/timeout/i.test(text)) {
    return 'Scanner timeout. Try improving lighting, holding the barcode flat, or enter the UPC manually.';
  }
